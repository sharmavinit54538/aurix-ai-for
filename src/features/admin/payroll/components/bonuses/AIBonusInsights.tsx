...
import { Sparkles, ArrowRight, Star, TrendingUp, ShieldAlert, Award } from "lucide-react";
...
export const AIBonusInsights: React.FC<AIBonusInsightsProps> = ({ insights }) => {
  const getSeverityBadge = (sev: BonusAIInsight["severity"]) => {
    switch (sev) {
      case "CRITICAL":
      case "WARNING":
        return <Badge className="bg-amber-500/10 text-amber-400 border-amber-500/30">Retention Alert</Badge>;
      case "SUCCESS":
        return <Badge className="bg-emerald-500/10 text-emerald-400 border-emerald-500/30">High Performer</Badge>;
      default:
  ...
  return (
    <div className="bns-card p-5 space-y-4 bg-slate-900/80 border border-amber-500/20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3.5">
        {insights.map((item) => (
          <div key={item.id} className="p-4 rounded-xl bg-slate-950/60 border border-white/5 space-y-2.5 hover:border-amber-500/30 transition-all">
            ...
            <div className="p-2 rounded bg-slate-900 border border-white/5 flex items-center justify-between text-[11px]">
              ...
              <span className="font-mono font-bold text-amber-300">{item.impactMetric}</span>
            </div>
            ...
            <div className="pt-2 flex items-center justify-between text-xs">
              ...
              <Button
                ...
                className="h-7 text-xs text-amber-400 hover:text-amber-300 gap-1 p-0"
              >
