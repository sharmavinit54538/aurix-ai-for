...
import { Sparkles, ArrowRight, ShieldAlert, CheckCircle2, AlertTriangle, Timer } from "lucide-react";
...
export const AIOvertimeInsights: React.FC<AIOvertimeInsightsProps> = ({ insights }) => {
  const getSeverityBadge = (sev: OvertimeAIInsight["severity"]) => {
    switch (sev) {
      case "CRITICAL":
        return <Badge className="bg-rose-500/10 text-rose-400 border-rose-500/30">Burnout Warning</Badge>;
      case "WARNING":
        return <Badge className="bg-amber-500/10 text-amber-400 border-amber-500/30">Cost Spike</Badge>;
      case "SUCCESS":
        return <Badge className="bg-emerald-500/10 text-emerald-400 border-emerald-500/30">Punch Verified</Badge>;
      default:
  ...
  return (
    <div className="ot-card p-5 space-y-4 bg-slate-900/80 border border-blue-500/20">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-sm font-semibold text-white flex items-center gap-2">
              Aurix AI Workforce Overtime & Burnout Intelligence
              <span className="ai-pulse-dot" />
            </h3>
            <p className="text-xs text-slate-400">Automated employee fatigue alerts, abnormal OT spikes (&gt; 50 hrs/mo), and turnstile punch verification.</p>
          </div>
        </div>
      </div>
        ...
        {insights.map((item) => (
          <div key={item.id} className="p-4 rounded-xl bg-slate-950/60 border border-white/5 space-y-2.5 hover:border-blue-500/30 transition-all">
            ...
            <div className="p-2 rounded bg-slate-900 border border-white/5 flex items-center justify-between text-[11px]">
              ...
              <span className="font-mono font-bold text-blue-300">{item.impactMetric}</span>
            </div>
