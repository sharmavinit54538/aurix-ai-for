...
import { OvertimeRecord, ApprovalStep } from "./overtimeTypes";
...
export const ApprovalWorkflowTracker: React.FC<ApprovalWorkflowTrackerProps> = ({
  ...
  const steps: ApprovalStep[] = record.approvalWorkflow || [
    { role: "Employee", name: record.employeeName, status: "APPROVED", timestamp: record.createdOn },
  ...
  return (
    <div className="ot-card p-5 space-y-4 bg-slate-900/80 border border-white/10">
      <div className="flex items-center justify-between pb-3 border-b border-white/10">
        <div>
          <h3 className="text-sm font-semibold text-white flex items-center gap-2">
            <ShieldCheck className="w-4 h-4 text-purple-400" />
            5-Stage Workforce Time Governance Tracker: <span className="text-blue-300 font-mono">{record.requestCode}</span>
          </h3>
          <p className="text-xs text-slate-400">Sequential sign-off flow from employee request to monthly salary credit.</p>
        </div>
        <Badge className="bg-purple-500/10 text-purple-300 border-purple-500/30 uppercase text-[10px]">
          Stage: {record.approvalStage}
        </Badge>
      </div>
          ...
          return (
            ...
            >
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400 mt-0.5">{step.timestamp || "Pending..."}</p>
              </div>

              {isPending && (
                <div className="pt-1 flex items-center gap-1 border-t border-white/5">
                  <Button
                    size="sm"
                    onClick={() => onApprove(record)}
                    className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white text-[10px] h-6"
                  >
                    Approve
                  </Button>
                </div>
