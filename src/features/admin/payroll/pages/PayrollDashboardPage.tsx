  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <StatCard label="Current cycle" value="Jul 2026" />
        <StatCard label="Total payout" value={formatCurrency(total)} />
        <StatCard label="Payslips ready" value={`${ws.employees.length} / ${ws.employees.length}`} />
      </div>
      ...
      <div className="mt-6 overflow-hidden rounded-2xl border border-border bg-card/60 backdrop-blur-xl">
        ...
        {ws.employees.length === 0 ? (
          <div className="p-12 text-center text-sm text-muted-foreground">
            Add employees to view their compensation.
          </div>
        ) : (
                ...
                {ws.employees.map((e) => {
                  ...
                  return (
                    <tr key={e.id} className="border-t border-border">
                      ...
                      <td className="px-4 py-3">{formatCurrency(net)}</td>
