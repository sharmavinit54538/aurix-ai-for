...
const INITIAL_SOURCED: SourcedCandidate[] = [
  {
    id: "src-101",
    name: "Arjun Krishnan",
    email: "arjun.k@techcorp.io",
    ...
    channel: "Email",
    notes: "Ex-Flipkart, strong TypeScript & distributed systems. Scheduled introductory call.",
  },
  {
    id: "src-102",
    name: "Divya Namboodiri",
    email: "divya.n@ai-labs.org",
    ...
    channel: "WhatsApp",
    notes: "Core contributor to open-source vLLM inference engine.",
  },
  {
    id: "src-103",
    name: "Rohan Varma",
    ...
    source: "Naukri",
    targetRole: "Lead Product Designer (UI/UX)",
    status: "Converted to Applicant",
    lastContacted: "2026-03-12",
    channel: "Email",
    notes: "Resume uploaded, moving to assessment stage.",
  },
  {
    id: "src-104",
    name: "Sneha Reddy",
    email: "sneha.reddy@cloudinfra.com",
    phone: "+91 96200 44556",
    source: "Referral",
    targetRole: "DevOps & Cloud Infrastructure Lead",
    status: "Sourced",
    lastContacted: "2026-03-16",
    channel: "SMS",
    notes: "Referred by Senior Engineering Manager Arun Verma.",
  },
];
...
const SOURCES = [
  ...
  },
  {
    ...
    desc: "Internal network sourcing with ₹50k referral bonuses.",
  },
  {
...
export function CandidateSourcingPage() {
  ...
  const [importRawNames, setImportRawNames] = useState(
    "Pooja Bansal, pooja.b@example.com, +91 98765 43210\nVikram Sethi, vikram.sethi@gmail.com, +91 99887 66554"
  );
  ...
  const handleOpenOutreach = (cand: SourcedCandidate) => {
    ...
    setComposerSubject(`Career Opportunity at OFC360: ${cand.targetRole} role`);
    setComposerBody(
      `Hi ${cand.name.split(" ")[0]},\n\nI came across your profile on ${cand.source} and was very impressed by your track record. We are expanding our team at OFC360 and looking for a ${cand.targetRole}.\n\nWould you be open for a brief 15-minute introductory call this week to explore this?\n\nBest regards,\nOFC360 Talent Acquisition Team`
    );
  ...
  const handleBulkImport = (e: React.FormEvent) => {
    ...
    const newItems: SourcedCandidate[] = lines.map((line, idx) => {
      ...
      return {
        ...
        email: parts[1] || `sourced.${Date.now()}@example.com`,
        phone: parts[2] || "+91 98000 11223",
        source: importSource,
        targetRole: importTargetRole,
        status: "Sourced",
        lastContacted: new Date().toISOString().split("T")[0],
        channel: "Email",
        notes: `Imported via bulk sourcing wizard from ${importSource}`,
      };
    });

    saveSourced([...newItems, ...sourcedList]);
    toast.success(`Imported ${newItems.length} prospective candidates into sourcing pipeline!`);
  ...
  const handleConvertToApplicant = (cand: SourcedCandidate) => {
    ...
    const newApplicant = {
      ...
      skills: ["Core Competency", "Problem Solving", "Domain Match"],
      yearsExperience: 5,
      ...
      feedback: [],
      notes: [{ id: "n-init", at: new Date().toISOString(), author: "Sourcing Bot", text: `Converted from sourcing lead on ${cand.channel}.` }],
      documents: [],
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Discover, import, and engage high-caliber passive talent across LinkedIn, GitHub, job boards, referrals, and staffing vendors."
        actions={
              ...
              {filteredSourced.map((cand) => (
                <tr key={cand.id} className="hover:bg-accent/30 transition-colors">
                  <td className="px-4 py-3">
                    ...
                    <div className="text-[11px] text-muted-foreground">{cand.email} • {cand.phone}</div>
                  </td>
