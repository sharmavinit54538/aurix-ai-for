...
import { Building2, Power, Search, ShieldCheck, UserCheck, UserX, Users, X } from "lucide-react";
...
export function SuperAdminUsersPage() {
  ...
  const refreshing = users.isFetching || statistics.isFetching;
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold tracking-tight text-foreground sm:text-3xl">User Management</h1>
            <Badge className="bg-purple-500/20 text-purple-300 border-purple-500/30 text-xs">Platform Registry</Badge>
          </div>
        ...
        <div className="flex items-center gap-3">
          <LastUpdated timestamp={users.dataUpdatedAt} />
          <RefreshButton onClick={refreshAll} refreshing={refreshing} />
        </div>
      </div>
      ...
      {/* Signed-in platform owner (identity verified by /auth/me) */}
      {ws.user && (
        <div className="rounded-xl border border-purple-500/30 bg-purple-950/20 p-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <span className="font-bold text-sm text-foreground">{ws.user.fullName || MISSING_VALUE}</span>
                <Badge className="bg-purple-500/30 text-purple-300 border-purple-500/40 text-[10px]">Signed-in Super Admin</Badge>
              </div>
              <div className="text-xs text-muted-foreground mt-0.5 font-mono">{ws.user.email}</div>
            </div>
          </div>
          <div className="text-[11px] text-muted-foreground sm:text-right">
            Activation controls are disabled for Super Admin accounts to prevent locking the platform owner out.
          </div>
        </div>
      )}
