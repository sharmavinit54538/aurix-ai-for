 ...
 * Connects directly to authenticated backend services.
 */
import { tool } from "ai";
import { z } from "zod";
...
export const hrTools = {
  ...
  getEmployee: tool({
    ...
    execute: async ({ idOrName }) => {
      try {
        ...
        const res = await apiInstance.get("/employees", { params: { search: needle, limit: 10 } });
        const items = res.data?.data?.items ?? res.data?.items ?? (Array.isArray(res.data) ? res.data : []);
  ...
  getLeaveBalance: tool({
    ...
    execute: async ({ employeeId }) => {
      try {
        const res = await apiInstance.get("/leaves/balances", { params: employeeId ? { employee_id: employeeId } : undefined });
  ...
  attritionRiskList: tool({
    ...
    execute: async ({ department, topN = 10 }) => {
      try {
        const res = await apiInstance.get("/ai-insights/attrition");
        const items = res.data?.data ?? res.data ?? [];
        const scope = Array.isArray(items) ? items : [];
        const filtered = department
          ? scope.filter((e: any) => String(e.department || "").toLowerCase() === department.toLowerCase())
          : scope;
        return { count: filtered.slice(0, topN).length, scope: department ?? "company", employees: filtered.slice(0, topN) };
      } catch {
  ...
  searchPolicies: tool({
    ...
    execute: async ({ query, topK = 3 }) => {
      try {
        const res = await apiInstance.get("/policy-assistant/search", { params: { q: query, limit: topK } });
        const results = res.data?.data ?? res.data ?? [];
        return { count: Array.isArray(results) ? results.length : 0, query, results: Array.isArray(results) ? results : [] };
      } catch {
