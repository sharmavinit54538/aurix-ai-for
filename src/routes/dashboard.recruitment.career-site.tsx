...
import { ExternalLink, Eye, Globe, Palette, Share2 } from "lucide-react";
...
function CareerSite() {
  const jobs = useRecruitment((s) => s.jobs.filter((j) => j.status === "active"));
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <>
            <Button variant="outline"><Share2 className="mr-2 h-4 w-4" />Copy URL</Button>
            <Button><ExternalLink className="mr-2 h-4 w-4" />Open Live</Button>
          </>
        }
      />
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-5">
        <div className="space-y-4 lg:col-span-2">
          <div className="rounded-2xl border border-border bg-card/60 p-4 backdrop-blur-xl">
            <div className="mb-3 flex items-center gap-2 text-sm font-semibold"><Palette className="h-4 w-4" />Branding</div>
            <div className="space-y-3">
              ...
              <Field label="Accent color">
                <div className="flex items-center gap-2">
                  <input type="color" value={accent} onChange={(e) => setAccent(e.target.value)} className="h-9 w-12 rounded-md border border-border bg-background" />
          ...
          <div className="rounded-2xl border border-border bg-card/60 p-4 backdrop-blur-xl">
            <div className="mb-3 flex items-center gap-2 text-sm font-semibold"><Globe className="h-4 w-4" />Visibility & Compliance</div>
            <div className="space-y-3">
        ...
        <div className="lg:col-span-3">
          ...
          <div className="overflow-hidden rounded-2xl border border-border bg-background shadow-sm">
            <div className="p-8" style={{ background: `linear-gradient(135deg, ${accent}22, transparent 60%)` }}>
              <div className="text-xs font-medium uppercase tracking-wider" style={{ color: accent }}>Careers at {brand}</div>
              <h1 className="mt-2 font-display text-3xl font-semibold tracking-tight">{tagline}</h1>
                  ...
                  <div className="flex items-center gap-3">
                    {showSalary && (
                      <span className="hidden text-xs text-muted-foreground sm:inline">
                        {j.currency} {(j.salaryMin / 1000).toFixed(0)}k–{(j.salaryMax / 1000).toFixed(0)}k
                      </span>
                    )}
            ...
            <div className="border-t border-border p-4 text-[11px] text-muted-foreground">{eeoStatement}</div>
          </div>
        </div>
      </div>
    </>
...
function Toggle({ label, checked, onCheckedChange }: { label: string; checked: boolean; onCheckedChange: (v: boolean) => void }) {
  return (
    <label className="flex items-center justify-between gap-3 rounded-lg border border-border bg-background/60 p-2 text-sm">
      <span>{label}</span>
