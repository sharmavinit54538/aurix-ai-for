...
import type { ID, Job, EmploymentType, WorkMode, JobStatus } from "@/lib/recruitment/types";

export const Route = createFileRoute("/dashboard/recruitment/jobs/new")({
  head: () => ({ meta: [{ title: "AI Job Creator — Aurix" }] }),
  component: NewJob,
});
...
function NewJob() {
  ...
  const handleSubmitJob = async (status: JobStatus) => {
    ...
    try {
      const response = (await recruitment.upsertJob(jobPayload)) as any;
